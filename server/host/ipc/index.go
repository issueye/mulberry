...
import (
	...
	taskService "carambola/host/app/task/service"
...
func (HostHelper) TaskComplete(ctx context.Context, cb *pb.TaskCallback) (*pb.Empty, error) {
	// 添加任务执行记录表中
	hSrv := taskService.NewHistory()
	hSrv.UpdateByField(
		"execution_id", cb.ExecutionID,
		map[string]any{
			"end_time":            time.Now().Format("2006-01-02 15:04:05"),
			"next_execution_time": cb.NextExecutionTime,
			"status":              cb.GetCode(),
			"result":              cb.GetMessage(),
		},
	)
	return &pb.Empty{}, nil
}

func (HostHelper) TaskStart(ctx context.Context, cb *pb.TaskCallback) (*pb.Empty, error) {
	hSrv := taskService.NewHistory()
	hSrv.Create(&model.TaskExecutionHistory{
		TaskExecutionHistoryBase: model.TaskExecutionHistoryBase{
