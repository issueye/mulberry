...
import (
	...
	"mulberry/internal/host/app/admin/response"
	"mulberry/internal/host/app/admin/service"
	taskService "mulberry/internal/host/app/task/service"
	"time"
...
func GetHomeCount() (map[string]any, error) {
	...
	clientCount, err := taskService.NewClient(global.DB, false).GetAllCount()
	if err != nil {
		return nil, err
	}

	dbCount, err := taskService.NewDatabaseInfo(global.DB, false).GetAllCount()
	if err != nil {
		return nil, err
	}

	taskCount, err := taskService.NewTask(global.DB, false).GetAllCount()
	if err != nil {
		return nil, err
	}
